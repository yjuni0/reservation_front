...
function OnlineCounselDetail() {
  ...
  const deleteNotice = async () => { 
    try {
      const response = await axios.delete(`/api/member/question/${questionId}/delete`);
      console.log(response);
      console.log('deleteOnlineCounsel seccess');
      if (response.status == 200) {
        alert("게시글을 삭제 하였습니다.");
        navigate("/notice");
      }
    } catch (error) {
      console.log("deleteOnlineCounsel error");
      console.error(error);
  ...
  useEffect(() => {
       console.log("noticeId:", questionId); // 콘솔로 확인
    if (!questionId) return;
  console.log("noticeId:", questionId);
  getBbsDetail();
}, []);
...
export default OnlineCounselDetail;
